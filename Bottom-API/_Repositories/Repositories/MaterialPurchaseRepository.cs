...
namespace Bottom_API._Repositories.Repositories
{
    public class MaterialPurchaseRepository : BottomRepository<WMSB_Material_Purchase>, IMaterialPurchaseRepository
    {
        ...
        public string GetFactoryID(string Purchase_No, string MO_No, string MO_Seq, string Material_ID)
        {
           var model = _context.WMSB_Material_Purchase.Where(x => x.Purchase_No.Trim() == Purchase_No.Trim() 
                        && x.MO_No.Trim() == MO_No.Trim() 
                        && x.MO_Seq.Trim() == MO_Seq.Trim() && x.Material_ID.Trim() == Material_ID.Trim()).FirstOrDefault();
           if (model != null)
           {
               return model.Factory_ID;
           }
           else 
           {
               return "";
