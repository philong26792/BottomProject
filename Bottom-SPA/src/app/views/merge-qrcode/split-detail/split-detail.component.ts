...
import { SplitDetail, SplitMain } from '../../../_core/_models/merge-qrcode/split-main';
...
export class SplitDetailComponent implements OnInit {
  ...
  detail(transacNo: string) {
    this.router.navigate(['/merge-qrcode/split/qrcode-detail-child', transacNo]);
  }

  print(splitDetail: SplitDetail) {
    this.outputService.changePrintQrCode('6');
  ...
  printParent(splitDetail: SplitDetail) {
    ...
    const paramPrintQrCodeAgain = [{
      qrCode_ID: splitDetail.qrCode_ID,
      qrCode_Version: splitDetail.transac_Type == 'MG' ? splitDetail.qrCode_Version : splitDetail.qrCode_Version + 1,
      mO_Seq:  splitDetail.mO_Seq
    }];
    this.outputService.changeParamPrintQrCodeAgain(paramPrintQrCodeAgain);
    this.router.navigate(['/output/print-qrcode-again']);
